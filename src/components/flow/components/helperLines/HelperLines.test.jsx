...
describe("HelperLines component", () => {
  it("renders nothing when there is no dragging node", () => {
    ...
    const { container } = render(
      <HelperLines nodes={[]} draggingNodeId={null} />,
    );
  ...
  it("handles dot node dragging producing center-based alignments", () => {
    ...
    const horizontal = Array.from(lines).find(
      (l) => l.getAttribute("x1") === "-50000",
    );
    const vertical = Array.from(lines).find(
      (l) => l.getAttribute("y1") === "-50000",
    );
