...
export const getNodeDimensions = ({ node, nodeFromState, isDot }) => {
  ...
  const width = CompareValuesWithSymbol(
    "||",
    node?.measured?.width,
    node?.width,
    nodeFromState?.width,
    nodeFromState?.data?.width,
    0,
  );
  const height = CompareValuesWithSymbol(
    "||",
    node?.measured?.height,
    node?.height,
    nodeFromState?.height,
    nodeFromState?.data?.height,
    0,
  );
  return { width, height };
};
...
export const addDotNodeCoords = ({
  ...
  if (!allVerticalCoords.has(centerX))
    allVerticalCoords.set(centerX, new Set());
  if (!allHorizontalCoords.has(centerY))
    allHorizontalCoords.set(centerY, new Set());
