...
import {addDotNodeCenterAlignments, addDotNodeCoords, calculateCanvasBounds, calculateDotNodeCenter, getNodeDimensions, isDotNode} from './HelperLines.functions'  
import { CompareValuesWithSymbol } from '../../../../utills/nodeNameUtils/nodeNameUtils';

function getNodePosition({ node, nodeFromState, isDraggingNode }) { //Helper function to get node position
  if (CompareValuesWithSymbol('&&', isDraggingNode, node?.internals?.positionAbsolute)) {
...
function roundCoord(coord) { //Helper function to round coordinates (handles floating point precision)
  return Math.round(coord * 100) / 100;
}

function addRegularNodeCoords({ nodeId, position, width, height, allHorizontalCoords, allVerticalCoords }) { //Add regular node coordinates to alignment maps
...
function findNodeById({ activeDraggingNodeId, nodeLookup, nodes }) { //Get node from lookup or nodes array
  return nodeLookup.get(activeDraggingNodeId) || nodes.find(n => n.id === activeDraggingNodeId);
}

function getDotNodeCenter({ activeDraggingNodeId, nodeLookup, nodes }) { //Get dot node center coordinates if dragging node is a dot node
...
function processNode({ //Process a single node and add its alignment coordinates
  nodeFromState,
  nodeFromLookup,
...
function collectAllNodes({ nodes, nodeLookup }) { // Collect all nodes to process from both sources
...
function shouldAddCoordinate({ coord, roundedDotCoord }) { //Check if coordinate should be added (not duplicate of dot node center)
  return roundedDotCoord === null || Math.abs(roundCoord(coord) - roundedDotCoord) > 0.01;
}

function processHorizontalAlignments({ allHorizontalCoords, roundedDotY, horizontalAlignments }) { //Process horizontal coordinate alignments
...
function processVerticalAlignments({ allVerticalCoords, roundedDotX, verticalAlignments }) { //Process vertical coordinate alignments
...
function extractAlignments({ allHorizontalCoords, allVerticalCoords, dotNodeCenterX, dotNodeCenterY }) { //Extract alignments from coordinate maps
...
function formatHelperLines({ horizontalAlignments, verticalAlignments, canvasBounds }) { //Remove duplicate alignments and format for rendering
...
export function HelperLines({ draggingNodeId = null, nodes = [] }) { //HelperLines component that renders alignment guides when nodes are being dragged
  const { getViewport } = useReactFlow();
  const nodeLookup = useStore((state) => state.nodeLookup);
  const width = useStore((state) => state.width);
  const height = useStore((state) => state.height);

  const detectedDraggingNodeId = useStore((state) => {  // Detect dragging node from store if not provided
  ...
  const helperLines = useMemo(() => {
    ...
    const { centerX: dotNodeCenterX, centerY: dotNodeCenterY } = getDotNodeCenter({  // Get dot node center if dragging a dot node
      activeDraggingNodeId,
      nodeLookup,
      nodes,
    });

    const { horizontalAlignments, verticalAlignments } = extractAlignments({ // Extract alignments
      allHorizontalCoords,
      allVerticalCoords,
      dotNodeCenterX,
      dotNodeCenterY,
