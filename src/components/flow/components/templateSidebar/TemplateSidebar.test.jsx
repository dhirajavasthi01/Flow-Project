...
test('toggles the template list visibility', async () => {
    ...
    expect(screen.queryByText('No saved templates yet. Select nodes and edges, then click')).not.toBeInTheDocument();
    fireEvent.click(toggleButton);
});

test('handles empty templates gracefully', () => {

    useTemplateManager.mockReturnValue({ templates: [], deleteTemplate: mockDelete });
    render(<TemplateSidebar />);
