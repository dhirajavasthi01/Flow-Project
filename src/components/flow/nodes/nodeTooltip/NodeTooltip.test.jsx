...
vi.mock("@xyflow/react", () => {
  return {
    NodeToolbar: vi.fn(
      ({ isVisible, className, children, position, tabIndex, ...rest }) => (
        <div
  ...
  it("triggers visibility on hover when IDs match", () => {
    ...
    render(
      <NodeTooltip>
        ...
        <NodeTooltipContent id={testId}>Tooltip Content</NodeTooltipContent>
      </NodeTooltip>,
    );
...
describe("NodeTooltipContent", () => {
  ...
  it("renders NodeToolbar with correct classes and props", () => {
    ...
    expect(toolbar.className).toContain(
      "bg-primary_gray text-primary_white rounded-[.3vmin] z-999 extra-class",
    );
