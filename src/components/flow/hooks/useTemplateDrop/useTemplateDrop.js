...
function calculateParentNodePosition(nodePosition, center, dropPosition, offset) {
  return {
...
function cloneNodeWithNewId(node, newId, dropPosition, center, offset, nodeIdMap) {
...
function cloneAllNodes(nodes, dropPosition, center, offset) {
  ...
  const clonedNodes = nodes.map(node => {
    ...
    return cloneNodeWithNewId(node, newId, dropPosition, center, offset, nodeIdMap);
  });
...
export const useTemplateDrop = () => {
  ...
  const cloneTemplate = useCallback((templateId, dropPosition, offset = { x: 20, y: 20 }) => {
    ...
    const nodesWithUpdatedParentIds = updateAllParentIds(clonedNodes, nodeIdMap);
